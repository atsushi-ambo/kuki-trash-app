    ...
    const regionData = {
        ...
        'kurihashi-e': {
            ...
            schedule: {
                ...
                '資源プラスチック類': { days: ['火曜日'], time: '8:30' },
                'びん・缶・ペットボトル': { days: ['火曜日'], time: '8:30' },
                '新聞・段ボール': { days: ['第1・第3金曜日'], time: '8:30' }
            }
        },
        'kurihashi-f': {
            name: '栗橋地区Fブロック',
            ...
            schedule: {
                ...
                '資源プラスチック類': { days: ['月曜日'], time: '8:30' },
                'びん・缶・ペットボトル': { days: ['月曜日'], time: '8:30' },
                '新聞・段ボール': { days: ['第2・第4金曜日'], time: '8:30' }
            }
        },
        'kurihashi-g': {
            name: '栗橋地区Gブロック',
    ...
    const LaunchRequestHandler = {
        ...
        handle(handlerInput) {
            const speechText = '久喜市ゴミ分別スキルへようこそ。お住まいの地区を設定すると、正確な収集日をお知らせできます。お住まいの地区とブロックを教えてください。例えば、「久喜地区Aブロック」、「久喜地区Dブロック」、「菖蒲地区Aブロック」のように具体的にお答えください。';
            const repromptText = 'お住まいの地区とブロックを教えてください。例えば、「久喜地区Dブロック」のように具体的にお答えください。';
    ...
    const GarbageSearchIntentHandler = {
        ...
        handle(handlerInput) {
            ...
            if (garbageItem && garbageData[garbageItem]) {
                ...
                // Handle schedule-only questions (何曜日, いつ, etc.)
                if (userLocation && regionData[userLocation]) {
                    const regionInfo = regionData[userLocation];
                    let scheduleResponse = `${regionInfo.name}の収集スケジュールをお知らせします。`;
                    
                    // Add main schedule information
                    const schedule = regionInfo.schedule;
                    if (schedule['燃やせるごみ']) {
                        scheduleResponse += ` 燃やせるごみは${schedule['燃やせるごみ'].days.join('と')}の${schedule['燃やせるごみ'].time}、`;
                    }
                    if (schedule['びん・缶・ペットボトル']) {
                        scheduleResponse += ` びん・缶・ペットボトルは${schedule['びん・缶・ペットボトル'].days.join('と')}の${schedule['びん・缶・ペットボトル'].time}、`;
                    }
                    if (schedule['資源プラスチック類']) {
                        scheduleResponse += ` 資源プラスチック類は${schedule['資源プラスチック類'].days.join('と')}の${schedule['資源プラスチック類'].time}です。`;
                    }
                    
                    speechText = scheduleResponse + ' 具体的なゴミについて詳しく知りたい場合は、ペットボトルや缶などと言ってください。';
                } else {
                    speechText = 'どのようなゴミについて知りたいですか？まず、お住まいの地区とブロックを教えてください。例えば、「久喜地区Dブロック」のように言ってください。';
                }
            }
    ...
    function extractLocationFromPhrase(phrase) {
        ...
        // Additional pattern matching for more flexible input
        const lowerPhrase = phrase.toLowerCase();
        
        // Match block letters more flexibly
        if (lowerPhrase.includes('久喜') || lowerPhrase.includes('くき')) {
            if (lowerPhrase.includes('a') || lowerPhrase.includes('エー') || lowerPhrase.includes('えー')) {
                return 'kuki-a';
            } else if (lowerPhrase.includes('b') || lowerPhrase.includes('ビー') || lowerPhrase.includes('びー')) {
                return 'kuki-b';
            } else if (lowerPhrase.includes('c') || lowerPhrase.includes('シー') || lowerPhrase.includes('しー')) {
                return 'kuki-c';
            } else if (lowerPhrase.includes('d') || lowerPhrase.includes('ディー') || lowerPhrase.includes('でぃー')) {
                return 'kuki-d';
            }
        }
        
        // Similar pattern for other regions
        if (lowerPhrase.includes('菖蒲') || lowerPhrase.includes('しょうぶ')) {
            if (lowerPhrase.includes('a') || lowerPhrase.includes('エー') || lowerPhrase.includes('えー')) {
                return 'shobu-a';
            } else if (lowerPhrase.includes('b') || lowerPhrase.includes('ビー') || lowerPhrase.includes('びー')) {
                return 'shobu-b';
        ...
        if (lowerPhrase.includes('鷲宮') || lowerPhrase.includes('わしのみや')) {
            if (lowerPhrase.includes('a') || lowerPhrase.includes('エー') || lowerPhrase.includes('えー')) {
                return 'washinomiya-a';
            } else if (lowerPhrase.includes('b') || lowerPhrase.includes('ビー') || lowerPhrase.includes('びー')) {
                return 'washinomiya-b';
            } else if (lowerPhrase.includes('c') || lowerPhrase.includes('シー') || lowerPhrase.includes('しー')) {
                return 'washinomiya-c';
            } else if (lowerPhrase.includes('d') || lowerPhrase.includes('ディー') || lowerPhrase.includes('でぃー')) {
                return 'washinomiya-d';
            }
        }
        
        if (lowerPhrase.includes('栗橋') || lowerPhrase.includes('くりはし')) {
            if (lowerPhrase.includes('e') || lowerPhrase.includes('イー') || lowerPhrase.includes('いー')) {
                return 'kurihashi-e';
            } else if (lowerPhrase.includes('f') || lowerPhrase.includes('エフ') || lowerPhrase.includes('えふ')) {
                return 'kurihashi-f';
            } else if (lowerPhrase.includes('g') || lowerPhrase.includes('ジー') || lowerPhrase.includes('じー')) {
                return 'kurihashi-g';
            } else if (lowerPhrase.includes('h') || lowerPhrase.includes('エイチ') || lowerPhrase.includes('えいち')) {
                return 'kurihashi-h';
            }
        }
    ...
    const SetLocationIntentHandler = {
        ...
        handle(handlerInput) {
            ...
            if (locationSlot && (locationSlot.value || (locationSlot.resolutions && locationSlot.resolutions.resolutionsPerAuthority))) {
                let locationCode = null;
                
                // First, try to extract location directly from the user's input
                if (locationSlot.value) {
                    locationCode = extractLocationFromPhrase(locationSlot.value);
                }
                
                // If direct extraction didn't work, try slot resolution as fallback
                if (!locationCode && locationSlot.resolutions && 
                    ...
                    // Try to find the best match by checking all resolved values
                    const resolvedValues = locationSlot.resolutions.resolutionsPerAuthority[0].values;
                    const originalInput = locationSlot.value || '';
                    
                    // Find the best match by looking for letter/block indicators in the original input
                    let bestMatch = null;
                    for (const valueObj of resolvedValues) {
                        const resolvedValue = valueObj.value.name;
                        const testLocationCode = extractLocationFromPhrase(resolvedValue);
                        
                        if (testLocationCode) {
                            // Check if this resolved value matches what the user actually said
                            if ((originalInput.includes('a') || originalInput.includes('A') || originalInput.includes('エー')) && resolvedValue.includes('A')) {
                                bestMatch = resolvedValue;
                                locationCode = testLocationCode;
                                break;
                            } else if ((originalInput.includes('b') || originalInput.includes('B') || originalInput.includes('ビー')) && resolvedValue.includes('B')) {
                                bestMatch = resolvedValue;
                                locationCode = testLocationCode;
                                break;
                            } else if ((originalInput.includes('c') || originalInput.includes('C') || originalInput.includes('シー')) && resolvedValue.includes('C')) {
                                bestMatch = resolvedValue;
                                locationCode = testLocationCode;
                                break;
                            } else if ((originalInput.includes('d') || originalInput.includes('D') || originalInput.includes('ディー')) && resolvedValue.includes('D')) {
                                bestMatch = resolvedValue;
                                locationCode = testLocationCode;
                                break;
                            } else if (!bestMatch) {
                ...
                if (locationCode && regionData[locationCode]) {
                    ...
                    const inputValue = locationSlot.value || '入力された値';
                    speechText = `申し訳ありません。${inputValue}は認識できませんでした。久喜地区、菖蒲地区、鷲宮地区、栗橋地区のいずれかを教えてください。例えば、「久喜地区Aブロック」や「菖蒲地区B」のように具体的に言ってください。`;
                }
            } else {
                speechText = 'お住まいの地区とブロックを教えてください。例えば、「久喜地区Aブロック」、「久喜地区Dブロック」、「菖蒲地区Aブロック」のように具体的にお答えください。';
            }
    ...
    const LaunchByNameIntentHandler = {
        ...
        handle(handlerInput) {
            const speechText = '久喜市ゴミ分別アプリへようこそ。お住まいの地区を設定すると、正確な収集日をお知らせできます。お住まいの地区とブロックを教えてください。例えば、「久喜地区Aブロック」、「久喜地区Dブロック」、「菖蒲地区Aブロック」のように具体的にお答えください。';
            const repromptText = 'お住まいの地区とブロックを教えてください。例えば、「久喜地区Dブロック」のように具体的にお答えください。';
